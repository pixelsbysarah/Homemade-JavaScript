...
* January 2016
* 
* -- Should I build the 0 and 1 animated slides via handlebar or grab them via DOM?
* 
*/
...
/*
 *
 //Model - should be JSON, but figure it out
 *<ul class="fifiSlider">
 *<li><img src="img" title="Title" /></li>
 **<li><img src="img" title="Title" /></li>
 ***<li><img src="img" title="Title" /></li>
 ****<li><img src="img" title="Title" /></li>
 *</ul>
 ...
 *
 *<div class="fifiSlider"></div>
 *
 //Views
 //javascript templating
 *<script id="slideTemplate">
 *<div class="slide">
	<img />
	<div class="content">
 ...
 */
 // Controller
// slider.js
// http://ejohn.org/blog/simple-javascript-inheritance/ how to write the controllers
...
var FifiSlide = Class.extend({
     ...
     /*---
     updateConstants: function(config){
          self.timer = config.timer || self.timer;
     },
     ----*/
     init: function(config){
          var self = this;
          //if (config) self.updateConstants(config);
          
          self.buildSlider();
          
          $(window).resize(function(){
               self.screenWidth = (window.innerWidth) ? window.innerWidth : document.documentElement.clientWidth;
               //self.sliderResize();
          });
     },
     buildSlider: function(){
          var self = this,
               ...
               animatedData = {slides:[this.slideData.slides[this.slideData.slides.length - 1], this.slideData.slides[0], this.slideData.slides[1]]},
               sliderTemplate = Handlebars.compile(fsView),
               compiledSlides = sliderTemplate(self.slideData),
               animatedSlides = sliderTemplate(animatedData);

          $('#'+self.fsContainer).html(compiledSlides);
          
          $('.arrow.right').on('click',function(){ self.nextSlide(); });
          $('.arrow.left').on('click',function(){ self.prevSlide(); });
          
          //self.sliderResize();
     },
     nextSlide: function(){
          //ADD: flag to check if animation in progress. avoids flickering if the button is spammed. reset flag when animation is complete.
          ...
          var self = this,
               ...
               players = $('#'+self.fsContainer+ ' .slide');

               $(players).eq(0).children('.content').fadeToggle();
               $(players).eq(0).animate({opacity: 0}, {queue: false, duration: 2000}, 'easeInOutQuad')
               .animate({right:'100%'}, 2900);
               
               $(players).eq(1).animate({
               		right:'0'
               	}, 1500, 'easeInOutQuad', function(){
               		$(this).children('.content').fadeToggle(function(){
               			self.buildSlider();
               		});
               	});
     ...
     prevSlide: function(){
          ...
          var self = this,
               ...
               players = $('#'+self.fsContainer+ ' .slide'),
               playerLast = slides.length-1;

               $(players).eq(0).children('.content').fadeToggle();
               $(players).eq(0).animate({opacity: 0}, {queue: false, duration: 2000}, 'easeInOutQuad')
               .animate({right:'-100%'}, 2900);
               
               $(players).eq(playerLast).animate({
               		right:'0'
               	}, 1500, 'easeInOutQuad', function(){
               		$(this).children('.content').fadeToggle(function(){
               			self.buildSlider();
     ...
     sliderResize: function(){
          var self = this;
          $('#'+self.fsContainer+' .sliderwrap').css( 'height', Math.floor(self.screenWidth * self.aspectRatio) );
